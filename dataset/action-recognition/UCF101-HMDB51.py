...
class Path(object):
    ...
    def db_dir(database):
        if database == 'ucf101':
            ...
            root_dir = '/home/bbb/shicaiwei/data/action_recognition/UCF101'
            # Save preprocess data into output_dir
            output_dir = '/home/bbb/shicaiwei/data/action_recognition/data/ucf101'

            return root_dir, output_dir
        elif database == 'hmdb51':
            # folder that contains class labels
            root_dir = '/home/icml/shicaiwei/data/action_recognition/HMDB51'
            output_dir = '/home/icml/shicaiwei/data/action_recognition/hmdb51'
...
class VideoDataset(Dataset):
    def __init__(self, dataset="ucf101", split='train', clip_len=64, resize_height=256, resize_width=None,
        ...
        print(self._check_preprocess(dataset), preprocess)
            ...
            for video in os.listdir(os.path.join(self.output_dir, 'train', video_class)):
                if len(os.listdir(os.path.join(self.output_dir, 'train', video_class, video))) == 0:
                    ...
                    video_name = os.path.join(os.path.join(self.output_dir, 'train', video_class, video),
                                              sorted(os.listdir(
                                                  os.path.join(self.output_dir, 'train', video_class, video)))[0])
    ...
    def _centercrop(self, buffer, type='whole'):
        ...
        if type == 'partical':
            time_index = (np.random.randint(buffer.shape[0] - self.clip_len)) if (
                    buffer.shape[0] > self.clip_len) else 0
            buffer = buffer[time_index:time_index + self.clip_len,
                     height_index:height_index + self.crop_height,
                     width_index:width_index + self.crop_width, :]
            return buffer
